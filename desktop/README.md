...
- Windows 安装包直链：[feedgrab-desktop-setup-0.1.23.exe](https://github.com/iBigQiang/feedgrab/releases/download/desktop-v0.1.23-20260831/feedgrab-desktop-setup-0.1.23.exe)
- 发布页：[desktop-v0.1.23-20260831](https://github.com/iBigQiang/feedgrab/releases/tag/desktop-v0.1.23-20260831)
...
| 文件 | 适合对象 | 说明 |
| --- | --- | --- |
| `feedgrab-desktop-setup-0.1.23.exe` | 普通用户 | 双击安装，自动创建开始菜单和桌面快捷方式。 |

本次安装包来自 `feedgrab-desktop` 分支，打包时间 / 构建时间 `2026-08-31 22:21`，签名状态：未签名。本地文件 SHA256：

```text
8563BCD4344D87CAEDA0F545AD4AA988CCDBBAC9734CB0447E9DF81D8F28B0EF
```

上方下载地址来自 GitHub API 返回的 `browser_download_url`，已用 `curl.exe -I -L` 核验，最终返回 `200 OK`，文件大小 `381046375` bytes。

开发者/便携版可在本地运行 `npm run pack:dev` 或 `npm run pack:all` 生成。后续每次重新打包正式安装包都会递增桌面端小版本号，避免多个安装包都显示同一个版本。

## 安装与启动

1. 从 GitHub Releases 下载 `feedgrab-desktop-setup-0.1.23.exe`。
2. 双击安装包，按提示选择安装目录。
...
- `REDDIT_DOWNLOAD_MEDIA=false`：默认沿用在线图片地址；设为 `true` 时把帖子和评论中的图片下载到笔记同级的 `attachments/{item_id}/` 并改写为相对路径，适合长期离线归档。

帖子和评论中的图片会以 Markdown 图片语法内嵌，在 Obsidian 预览模式下直接显示；非图片链接保持普通链接不变。旧版本抓取的笔记不会自动改写，需要重新抓取才会变成内嵌图片。
